...
module.exports = function (myMoney) {
  ...
  class MyMoneyEmitter extends EventEmitter {
    constructor () {
      super();
    }
  }

  let events = myMoney.events = new MyMoneyEmitter();
  ...
  events.addListener("resource:created", function (uri, createdByUserId) {
    async(function () {
      myMoney.authz.registerOwnership(uri, createdByUserId);
      if (uri.split(/\//g).length !== 5) {
        return;
      }
      myMoney.authz.registerOwnership("/"+uri.split(/\//g).slice(3,2).join("/"), createdByUserId);
    });
  });
